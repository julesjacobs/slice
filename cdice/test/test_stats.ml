open OUnit2
module Distributions = Contdice.Distributions
...
let suite =
  "Stats Tests" >::: [
    "Uniform PDF vs CDF derivative - Classic (1,3)" >:: (fun _ ->
      let dist = Distributions.Uniform (1.0, 3.0) in
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
    ...
    "Uniform PDF vs CDF derivative - Negative (-2,-1)" >:: (fun _ ->
      let dist = Distributions.Uniform (-2.0, -1.0) in
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
    ...
    "Uniform PDF vs CDF derivative - Degenerate (2,2)" >:: (fun _ ->
      let dist = Distributions.Uniform (2.0, 2.0) in
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
    ...
    "Gaussian PDF vs CDF derivative - Classic (5,2)" >:: (fun _ ->
      let dist = Distributions.Gaussian (5.0, 2.0) in
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
    ...
    "Gaussian PDF vs CDF derivative - Standard Normal (0,1)" >:: (fun _ ->
      let dist = Distributions.Gaussian (0.0, 1.0) in
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
    ...
    "Exponential PDF vs CDF derivative - Lambda 1.5" >:: (fun _ ->
      let dist = Distributions.Exponential 1.5 in (* Mean = 0.667, StdDev = 0.667 *)
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
    ...
    "Exponential PDF vs CDF derivative - Lambda 0.5" >:: (fun _ ->
      let dist = Distributions.Exponential 0.5 in (* Mean = 2.0, StdDev = 2.0 *)
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
    ...
    "LogNormal PDF vs CDF derivative - Classic (1.0, 0.5)" >:: (fun _ ->
      ...
      let dist = Distributions.LogNormal (mu, sigma) in (* Median=exp(1)=2.718, Mode=exp(1-0.25)=2.117, Mean=exp(1+0.125)=3.080 *)
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
    ...
    "LogNormal PDF vs CDF derivative - (0.0, 1.0)" >:: (fun _ ->
      ...
      let dist = Distributions.LogNormal (mu, sigma) in (* Median=exp(0)=1, Mode=exp(0-1)=0.367, Mean=exp(0+0.5)=1.648 *)
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
    ...
    "Beta PDF vs CDF derivative - Classic (2.0, 5.0)" >:: (fun _ ->
      ...
      let dist = Distributions.Beta (a,b) in
      let cdf x = Distributions.cdistr_cdf dist x in
      let pdf x = Distributions.cdistr_pdf dist x in
      let atol, rtol = 1e-8, 1e-3 in
