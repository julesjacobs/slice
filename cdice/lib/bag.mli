...
module Make (L : Lat) : sig
  ...
  (* Enforce that bag b1 is less than or equal to bag b2 in the lattice. This
     updates b2 with the union of b1 and b2, and sets up propagation so future
     changes to b1 also update b2. *)
  val leq : bag -> bag -> unit

  (* Enforce that bag b1 and bag b2 are equal in the lattice. Equivalent to leq
     b1 b2 and leq b2 b1. *)
  val eq : bag -> bag -> unit
  ...
  (* Register a listener function to be called whenever the bag's value
     changes. *)
