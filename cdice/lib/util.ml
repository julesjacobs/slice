...
let rec string_of_expr_indented ?(indent=0) ?(max_discr=0) e =
  string_of_expr_node ~indent ~max_discr e
and string_of_aexpr_indented ?(indent=0) ?(max_discr=0) ae =
  string_of_aexpr_node ~indent ~max_discr ae
and string_of_texpr_indented ?(indent=0) ?(max_discr=0) ((ty, aexpr) : texpr) : string =
  let aexpr_str = string_of_aexpr_indented ~indent ~max_discr aexpr in
  match aexpr with
  | TAExprNode (FinConst (_, _)) -> aexpr_str
  | _ -> Printf.sprintf "(%s : %s)" aexpr_str (string_of_ty ty)

and string_of_expr_node ?(indent=0) ?(max_discr=0) (ExprNode expr_node) : string =
  ...
  | Let (x, e1, e2) ->
      ...
      let e1_str = string_of_expr_indented ~indent:(indent+2) ~max_discr e1 in
      let e2_str = string_of_expr_indented ~indent:(indent+2) ~max_discr e2 in
  ...
  | DistrCase cases ->
      ...
      in
      let case_strs = List.map format_case cases in
      (* Ensure we have exactly max_discr values by padding with "0." if needed *)
      let padded_case_strs =
        if max_discr > List.length case_strs then
          let padding = List.init (max_discr - List.length case_strs) 
                        (fun _ -> "0.") in
          case_strs @ padding
        else
          case_strs
      in
      Printf.sprintf "discrete(%s)" (String.concat ", " padded_case_strs)
  | Less (e1, e2) ->
      Printf.sprintf "%s < %s" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)
  | LessEq (e1, e2) ->
      Printf.sprintf "%s <= %s" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)
  | And (e1, e2) ->
      Printf.sprintf "%s && %s" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)
  | Or (e1, e2) ->
      Printf.sprintf "%s || %s" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)
  | Not e1 ->
      Printf.sprintf "! %s" (string_of_expr_indented ~indent ~max_discr e1)
  | If (e1, e2, e3) ->
      let indent_str = String.make indent ' ' in
      let next_indent_str = String.make (indent+2) ' ' in
      let e1_str = string_of_expr_indented ~indent ~max_discr e1 in
      let e2_str = string_of_expr_indented ~indent:(indent+2) ~max_discr e2 in
      let e3_str = string_of_expr_indented ~indent:(indent+2) ~max_discr e3 in
      Printf.sprintf "if %s then\n%s%s\n%selse\n%s%s" e1_str next_indent_str e2_str indent_str next_indent_str e3_str
  | Pair (e1, e2) -> Printf.sprintf "(%s, %s)" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)
  | First e -> Printf.sprintf "(fst %s)" (string_of_expr_indented ~indent ~max_discr e)
  | Second e -> Printf.sprintf "(snd %s)" (string_of_expr_indented ~indent ~max_discr e)
  | Fun (x, e) ->
      let e_str = string_of_expr_indented ~indent:(indent+2) ~max_discr e in
      Printf.sprintf "fun %s -> %s" x e_str
  | App (e1, e2) -> Printf.sprintf "(%s %s)" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)
  | FinConst (k, _) -> Printf.sprintf "int(%d,%d)" (bit_length (max_discr-1)) k
  | FinLt (e1, e2, _) -> Printf.sprintf "%s < %s" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)
  | FinLeq (e1, e2, _) -> Printf.sprintf "%s <= %s" (string_of_expr_indented ~indent ~max_discr e1) (string_of_expr_indented ~indent ~max_discr e2)

and string_of_aexpr_node ?(indent=0) ?(max_discr=0) (TAExprNode ae_node) : string =
  match ae_node with
  | Const f -> Printf.sprintf "%g" f
  | BoolConst b -> string_of_bool b
  | Var x -> x
  | Let (x, te1, te2) ->
      let indent_str = String.make indent ' ' in
      let e1_str = string_of_texpr_indented ~indent:(indent+2) ~max_discr te1 in
      let e2_str = string_of_texpr_indented ~indent:(indent+2) ~max_discr te2 in
      Printf.sprintf "let %s = %s in\n%s%s" x e1_str indent_str e2_str
  | CDistr dist -> string_of_cdistr dist
  | DistrCase cases ->
      let format_case (_, prob) =
      ...
      in
      Printf.sprintf "discrete(%s)" (String.concat ", " padded_case_strs)
  | Less (te1, te2) -> Printf.sprintf "%s < %s" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
  | LessEq (te1, te2) -> Printf.sprintf "%s <= %s" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
  | And (te1, te2) ->
      Printf.sprintf "%s && %s" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
  | Or (te1, te2) ->
      Printf.sprintf "%s || %s" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
  | Not te1 ->
      Printf.sprintf "! %s" (string_of_texpr_indented ~indent ~max_discr te1)
  | If (te1, te2, te3) ->
      let indent_str = String.make indent ' ' in
      let next_indent_str = String.make (indent+2) ' ' in
      let e1_str = string_of_texpr_indented ~indent ~max_discr te1 in
      let e2_str = string_of_texpr_indented ~indent:(indent+2) ~max_discr te2 in
      let e3_str = string_of_texpr_indented ~indent:(indent+2) ~max_discr te3 in
      Printf.sprintf "if %s then\n%s%s\n%selse\n%s%s" e1_str next_indent_str e2_str indent_str next_indent_str e3_str
  | Pair (te1, te2) -> Printf.sprintf "(%s, %s)" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
  | First te -> Printf.sprintf "(fst %s)" (string_of_texpr_indented ~indent ~max_discr te)
  | Second te -> Printf.sprintf "(snd %s)" (string_of_texpr_indented ~indent ~max_discr te)
  | Fun (x, te) ->
      let e_str = string_of_texpr_indented ~indent:(indent+2) ~max_discr te in
      Printf.sprintf "fun %s -> %s" x e_str
  | App (te1, te2) -> Printf.sprintf "(%s %s)" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
  | FinConst (k, _) -> Printf.sprintf "int(%d,%d)" (bit_length (max_discr-1)) k
  | FinLt (te1, te2, _) -> Printf.sprintf "%s < %s" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
  | FinLeq (te1, te2, _) -> Printf.sprintf "%s <= %s" (string_of_texpr_indented ~indent ~max_discr te1) (string_of_texpr_indented ~indent ~max_discr te2)
...
let string_of_expr expr max_discr = string_of_expr_indented ~max_discr expr 
let string_of_texpr texpr max_discr = string_of_texpr_indented ~max_discr texpr
let string_of_aexpr aexpr max_discr = string_of_aexpr_indented ~max_discr aexpr
