...
The type \codetype{float[<0.5; T]} for the uniform sample indicates that the relevant comparison cutpoint is ``$<\!\!0.5$'' and the quantity can take on a continuous range ($\top$). This type allows \Slice{} to map \texttt{uniform(0,1)} in the original program to \texttt{discrete(0.5, 0.5)} in the discrete program.
The type \codetype{float[<0.5; 0.5]} for the constant $0.5$ indicates that the relevant comparison cutpoint is $< 0.5$ and the quantity can only take on the single constant value $0.5$. This type allows \Slice{} to map the constant 0.5 in the original program to the index 1 in the discrete program.
...
The cut points are determined by \emph{how the expression is used} (i.e., its context) and the value set is determined by \emph{the constants used in the expression}. In general, all continuous distributions with comparisons to a finite number of floats will have type \codetype{float[<c\_1, ..., <c\_n; T]}, where the cutpoints in this type allow \Slice{} to determine precisely where to partition the continuous range ($\top$) into intervals, in turn allowing it to determine the parameters in the \texttt{discrete} distribution in the discrete program by taking the cdfs of the continuous distribution at the various intervals. Essentially, the discretization is driven by the cut points: a float value with $n$ cut points is discretized into $n+1$ possible discrete values. 
...
Based on these type annotations, \Slice{} replaces the continuous distribution for \texttt{x} with a discrete one that has three outcomes, where the probability of each outcome is the probability mass of the original uniform distribution within the corresponding interval. This results in \texttt{discrete(0.1, 0.4, 0.5)}.
