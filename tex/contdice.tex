...
Not all programs can be fully discretized. Consider a program where continuous values are used in ways other than comparisons:
...
\noindent Since the continuous values are added rather than compared, there are no comparison points to drive discretization. In such cases, \Slice performs partial discretization: it discretizes the parts of the program that can be discretized while leaving other parts continuous. This allows for hybrid inference approaches where discrete parts can benefit from exact inference while continuous parts use approximate methods.
