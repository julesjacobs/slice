...
The type \lstinline{float[<0.5; T]} for the uniform sample indicates that the relevant comparison cutpoint is "$<\!\!0.5$" and the quantity can take on a continuous range ($\top$). 
The type \lstinline{float[<0.5; 0.5]} for the constant $0.5$ indicates that the relevant comparison cutpoint is $< 0.5$ and the quantity can only take on the single constant value $0.5$. 
...
let y = uniform(0, 2) : float[<0.1; T] in
if x < (0.5 : float[<0.1,<0.5; 0.5]) then
  x < (0.1 : float[<0.1,<0.5; 0.1])
else
  y < (0.1 : float[<0.1; 0.1])
    \end{lstlisting}
...
\begin{lstlisting}
let x =
    if (uniform(0, 1) : float[<0.5; T]) < 
       (0.5 : float[<0.5; 0.5]) then
        (0.3 : float[<0.5; 0.3])
    else
        (0.6 : float[<0.5; 0.6])
    : float[<0.5; 0.3,0.6]
...
gaussian(0, 
  if (uniform(0,1) : float[<0.5; T]) < 
    ...
    (2.4 : float[<=1.2,<=2.4; 2.4])
  : float[<=1.2,<=2.4; 1.2,2.4])
: float[<0.5; T] 
< (0.5 : float[<0.5; 0.5])
\end{lstlisting}
    \caption{Type annotations}
...
\begin{subfigure}[t]{0.48\textwidth}
    \begin{lstlisting}
let _h_sigma1 = if discrete(0.5, 0.5) < 1 then
    0
  else
    1 in
if _h_sigma1 == 0 then
    discrete(0.661539, 0.338461)
  else
    discrete(0.582516, 0.417484) < 1
    \end{lstlisting}
...
\begin{subfigure}[t]{0.48\textwidth}
\begin{lstlisting}
let x = uniform(0, 1) : float[<0.2,<0.5; T] in
let _ = observe(x < (0.5 : float[<0.2,<0.5; 0.5])) in
